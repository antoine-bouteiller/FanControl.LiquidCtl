...
public class LiquidctlPluginTests
{
	...
	public void Initialize_ShouldNotThrow()
	{
		...
		// Act
		Action act = () => plugin.Initialize();

		// Assert
		// Note: This will actually try to start the bridge process
		// In a real test environment, we would mock the bridge wrapper
		act.Should().NotThrow();
	}
