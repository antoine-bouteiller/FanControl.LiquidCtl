...
public class LiquidctlBridgeWrapperTests
{
	...
	[Fact]
	public void GetStatuses_WithoutInit_ShouldReturnEmptyCollection()
	{
		// Arrange
		var mockLogger = new Mock<IPluginLogger>();
		var wrapper = new LiquidctlBridgeWrapper(mockLogger.Object);
		...
		// Act
		var result = wrapper.GetStatuses();

		// Assert
		// Without proper initialization and bridge process, this should handle errors gracefully
		result.Should().NotBeNull();
	}

	[Fact]
	public void Dispose_AfterGetStatuses_ShouldNotThrow()
	{
		// Arrange
		var mockLogger = new Mock<IPluginLogger>();
		var wrapper = new LiquidctlBridgeWrapper(mockLogger.Object);

		// Try to get statuses (will fail but shouldn't crash)
		_ = wrapper.GetStatuses();
